...
print('</head>')
print('<body>')
print('<h2>Remove Product</h2>')

print('<h3>Choose a Product to be removed:</h3>')
connection = None
try:
    ...
    sql = 'SELECT  SKU, name, description, price, ean FROM product;'
    cursor.execute(sql)
    ...
    for row in result:
        ...
        print('<td><a href="set_remove_products.cgi?SKU={}" class="button">X</a></td>'.format(row[0]))
        print('</tr>')
    print('</table>')
